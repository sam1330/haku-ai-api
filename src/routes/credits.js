    ...
    if (event.type === 'checkout.session.completed') {
      ...
      await db.transaction(async (trx) => {
        ...
        if (payment && payment.status === 'succeeded') {
          console.log(`Payment ${session.id} already processed (status check)`);
          return;
        }
        ...
        if (existingTx) {
          console.log(`Credits already granted for session ${session.id} (tx check)`);
          return;
        }
      ...
      console.log(`Successfully processed payment for user ${userId}, plan ${plan_name}`);
    }
