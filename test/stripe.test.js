...
const { stripe, createCheckoutSession } = require('../src/services/stripeService');
...
describe('Stripe Integration Tests', () => {
    ...
    describe('POST /api/credits/create-checkout-session', () => {
        test('should create a checkout session and a pending payment record', async () => {
            ...
            const payment = await db('payments').where('stripe_checkout_session_id', mockSession.id).first();
    ...
    describe('POST /api/credits/webhook', () => {
        ...
        test('should process successful payment and grant credits', async () => {
            ...
            const initialCredits = await db('users').where('id', user.id).select('credits').first().then(u => u.credits);
            ...
            const payment = await db('payments').where('stripe_checkout_session_id', sessionId).first();
            const transaction = await db('credit_transactions').where('user_id', user.id).orderBy('created_at', 'desc').first();
        ...
        test('should be idempotent and not grant credits twice', async () => {
            ...
            const balanceAfterFirst = await db('users').where('id', user.id).select('credits').first().then(u => u.credits);
            ...
            const balanceAfterSecond = await db('users').where('id', user.id).select('credits').first().then(u => u.credits);
