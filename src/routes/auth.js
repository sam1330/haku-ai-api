...
const { validate, registerSchema, loginSchema, forgotPasswordSchema } = require('../middleware/validation');
...
router.post('/forgot-password', validate(forgotPasswordSchema), async (req, res, next) => {
  ...
  if (!email) {
    ...
    const user = await db('users').select('id', 'first_name').where('email', email).first();
    ...
    // Send Email
    const emailService = new EmailService();
    const emailResponse = await emailService.sendPasswordResetEmail(email, user.first_name, verificationToken);
